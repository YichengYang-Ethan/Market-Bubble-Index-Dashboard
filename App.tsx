...
const App: React.FC = () => {
  ...
  // Load bubble data when switching to bubble tab
  useEffect(() => {
    if (activeTab === 'bubble' && !bubbleData && !bubbleLoading) {
      loadBubbleData();
    }
  }, [activeTab, bubbleData, bubbleLoading, loadBubbleData]);
    ...
    <div className="min-h-screen pb-20 bg-slate-950 text-slate-100">
          ...
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xl font-bold text-white tracking-tight hidden sm:inline">Market Dashboard</span>
              </div>
            ...
            {/* Ticker Selector - only visible on Deviation tab */}
            {activeTab === 'deviation' && (
              <div className="flex items-center gap-1 bg-slate-800 rounded-lg p-1">
      ...
      {activeTab === 'deviation' ? (
        ...
        <>
          ...
          <header className="bg-slate-900/50 border-b border-slate-800 mb-8 py-10 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
              <h1 className="text-3xl font-black text-white mb-2">Bubble Index</h1>
              <p className="text-lg text-slate-400 max-w-2xl">
          ...
          <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            {bubbleLoading ? (
              <div className="flex items-center justify-center py-20">
                <div className="text-center">
                  <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                  <p className="text-slate-400 font-medium">Loading Bubble Index...</p>
                </div>
              </div>
            ) : bubbleError ? (
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2">
                    <IndicatorBreakdown indicators={bubbleData.indicators} />
                  </div>
                </div>
                ...
                <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 shadow-xl">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
                      <p className="text-emerald-400 font-bold text-sm">LOW (0-30)</p>
                      <p className="text-slate-400 text-xs mt-1">Depressed sentiment. Potential accumulation zone.</p>
                    </div>
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3">
                      <p className="text-yellow-400 font-bold text-sm">MODERATE (30-50)</p>
                      <p className="text-slate-400 text-xs mt-1">Normal market conditions. Balanced risk/reward.</p>
                    </div>
                    <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-3">
                      <p className="text-orange-400 font-bold text-sm">ELEVATED (50-70)</p>
                      <p className="text-slate-400 text-xs mt-1">Rising euphoria. Tighten risk management.</p>
                    </div>
                    <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                      <p className="text-red-400 font-bold text-sm">HIGH (70-85)</p>
                      <p className="text-slate-400 text-xs mt-1">Frothy conditions. Consider reducing exposure.</p>
                    </div>
                    <div className="bg-red-600/10 border border-red-600/20 rounded-lg p-3">
                      <p className="text-red-300 font-bold text-sm">EXTREME (85+)</p>
                      <p className="text-slate-400 text-xs mt-1">Bubble territory. High correction probability.</p>
                    </div>
                  </div>
                </div>
              </div>
